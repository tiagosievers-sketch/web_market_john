...
document.addEventListener("DOMContentLoaded", function () { 
    ...
    // Verifica o estado inicial e ajusta a exibição do campo
    if (checknoticiaEmail.checked) {
        enviarEmailTexto.style.display = 'block'; // Mostrar inicialmente
    } else {
        enviarEmailTexto.style.display = 'none'; // Esconder inicialmente
    }

    // Adiciona os eventos para mudança de comportamento
    checknoticiaPapel?.addEventListener('change', function () {
        if (this.checked) {
            enviarEmailTexto.style.display = 'none'; // Esconde quando "Papel" é selecionado
        }
    });

    checknoticiaEmail?.addEventListener('change', function () {
        if (this.checked) {
            enviarEmailTexto.style.display = 'block'; // Mostra quando "Email" é selecionado
        }
    });
});
